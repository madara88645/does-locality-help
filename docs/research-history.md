...
This is the former README, preserved on 11 September 2026 for traceability.
Source README blob: `67085f3cfa6d1fd1445858a57a27cc43e282b1d2`. Relative Markdown links have been adjusted for this document's location.

**Read the [current README](../README.md) for the present summary and scope.**
Some original interpretations below were too strong: the interpolated residual and
“59% explained” estimate are exploratory, not a causal decomposition or a fully
audited paired-uncertainty result. The original plan said no claim would rest on
interpolation; the later analysis departed from that plan. A stable point estimate
does not by itself refute sampling noise. The later class-incremental experiment
was performed and saturated; the older “absent here” scope paragraph is outdated.
These qualifications also apply when reading the linked original exploratory notes.

No new experiment or numerical reanalysis accompanies this editorial revision.
The original narrative follows; its historical wording is not a fresh endorsement.
...
**Step one, the [γ sweep](exploratory-gamma.md).** I varied the feedback strength
...
two rules were near-twins" ([`docs/limits.md`](limits.md)).

**Step two, [untied feedback](exploratory-untied.md).** I gave the top-down path its
...
credit assignment. **Step three, [the plasticity curve](exploratory-plasticity.md),**
...
([`docs/exploratory-untied.md`](exploratory-untied.md)).
...
[`docs/exploratory-plasticity.md`](exploratory-plasticity.md). One located the effect.
...
possible 99 ([`docs/exploratory-class-incremental.md`](exploratory-class-incremental.md)).
...
recording. See Amendment 1 in [`docs/preregistration.md`](preregistration.md), which
...
[`docs/preregistration.md`](preregistration.md) records the change, and the
...
See [`docs/limits.md`](limits.md) for which theorem needs which limit, and why CHL's
...
silently drops the `γ^(k-L)` term (see [`docs/limits.md`](limits.md)).
